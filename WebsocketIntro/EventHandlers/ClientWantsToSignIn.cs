...
public class ClientWantsToSignInEventHandler(SecurityService.SecurityService securityService) : BaseEventHandler<ClientWantsToSignInDto>

{
    public async override Task Handle(ClientWantsToSignInDto dto, IWebSocketConnection socket)
    {
        ...
        {
            Jwt = securityService.GenerateJwt(dto.Username)
        });
    }
}